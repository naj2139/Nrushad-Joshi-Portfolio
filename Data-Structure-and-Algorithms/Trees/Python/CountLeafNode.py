...
def count_leaf_node(root: TreeNode | None) -> int:
    """
    Return the total number of leaf nodes in a binary tree.

    A leaf node is a node with no left or right child.

    """

    # An empty tree (or empty subtree) contains no leaf nodes.
    if not root:
        return 0

    # If both children are missing, this node is a leaf.
    # Count it as one and stop exploring this path.
    if not root.right and not root.left:
        return 1

    # Otherwise, recursively count the leaf nodes in the
    # left and right subtrees and combine the results.
    return count_leaf_node(root.right) + count_leaf_node(root.left)

def main() -> None:
    root = build_example_tree()
    print("Total Leaf Node Count:", count_leaf_node(root))
